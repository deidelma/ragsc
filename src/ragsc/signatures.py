...
def get_gene_signature(adata: ad.AnnData,
    ...
    logger.trace("found {} genes with threshold great than {}", len(names), expression_threshold)
    assert expression.shape == names.shape
    if verbose:
        print(f"Found {len(names)} genes exceeding expression threshold {expression_threshold:.3f}.")
    ...
    logger.trace("{} genes remain after removing mitochondrial genes (MT-*)", len(genes))
    if verbose:
